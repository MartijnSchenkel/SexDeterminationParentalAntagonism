...
# Loop over types of novel SD gene (male-determining versus female-determining)
for(MaleDeterminer in c(T, F))
{
# ------------------- #
# Parameter values ---- 
# ------------------- #
...
NovelSexDeterminer <- MaleDeterminer
...
d2 <- d %>% dplyr::mutate(SD = str_sub(Haplotype, 1,1), PA = str_sub(Haplotype,2,2),
...
pm <- d6 %>% filter(MaleDeterminer == T) %>% 
  ...
  scale_x_continuous(limits = c(0,max_t), expand = c(0,0),
                     breaks = seq(0,50000, 10000), labels = seq(0,5,1)) + 
  scale_color_viridis(discrete = T, option = "plasma", begin = 0.1, end = 0.9,
  ...
  guides(color = guide_legend(legend.position = "bottom",
                              title.position = "top",
...
# Plot results from invasion of female-determining A (plot 2B)
pf <- d6 %>% filter(MaleDeterminer == F) %>% 
  ggplot(aes(Generation, Frequency, color = LL, linetype = Origin)) + 
  geom_vline(xintercept = 10000, linetype = 3, size = 1, color = rgb(0.4,0.4,0.4)) + 
  geom_line(size = 1) + 
  facet_wrap(~Sex) +
...
pf
# Extract legend
legend <- get_legend(pm)

# Combine subfigures into main figure
p <- plot_grid(pm + theme(legend.position = "none"),
          pf + theme(legend.position = "none"), 
          legend, 
          ncol = 1, rel_heights = c(1,1,0.3), labels = c("A", "B", "")
          )
...
# Save to PNG
png(file = "Supplementary_Figure_1.png", height = 6, width = 8, res = 1000, units = "in")
p
dev.off()


# Save to PDF
pdf(file = "Supplementary_Figure_1.pdf", height = 6, width = 8)
p
dev.off()
