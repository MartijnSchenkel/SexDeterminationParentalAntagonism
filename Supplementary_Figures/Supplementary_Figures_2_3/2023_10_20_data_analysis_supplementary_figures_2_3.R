...
dpp <- dpp %>% dplyr::mutate(fCY = str_c("italic(c)[1]==", CY),
                     fCA = str_c("italic(c)[2]==", CA))

write.table(x = dpp, col.names = T, row.names = F,
            file = "2023_10_20_Masculinizer_CY_CA_AY_AA_predictions.txt")
dpp <- read.table("2023_10_20_Masculinizer_CY_CA_AY_AA_predictions.txt", T)
summary(dpp)
...
pm <- dpp %>% filter(CA < -1.1) %>% ggplot(aes(AY, AA, z = Turnover)) + 
  ...
  labs(x = bquote(italic(a)[1]), y = bquote(italic(a)[2]), fill = bquote(italic(c)[2])) + 
...
png(file = "2023_10_20_Masculinizer_CY_CA_AY_AA.png", width = 8, height = 2, units = "in", res = 1000)
pm
dev.off()


pdf(file = "2023_10_20_Masculinizer_CY_CA_AY_AA.pdf", width = 8, height = 2)
pm
dev.off()
...
write.table(x = dpp, col.names = T, row.names = F,
            file = "2023_10_20_Feminizer_CY_CA_AY_AA_predictions.txt")
dpp <- read.table("2023_10_20_Feminizer_CY_CA_AY_AA_predictions.txt", T)
summary(dpp)
...
pf <- dpp %>% ggplot(aes(AY, AA)) + 
  ...
  scale_x_continuous(limits = c(min(dpp$AY), max(dpp$AY)), expand = c(0,0),
                     ...
                     labels = c("-1", "-0.75", "-0.5", "-0.25", "0")) + 
  scale_y_continuous(limits = c(min(dpp$AA), max(dpp$AA)), expand = c(0,0)) + 
  labs(x = bquote(italic(a)[1]), y = bquote(italic(a)[2]), fill = "SD gene" ) + 
  scale_fill_viridis(discrete = T, option = "magma", begin = 0.9, end = 0.1, 
                     labels = c(bquote(italic(s)[22]),bquote(italic(s)[12]),bquote(italic(s)[12]*"and"*(italic(s)[22])))) + 
  theme(panel.spacing = unit(1, "lines"),
        strip.background = element_rect(fill = rgb(0.2,0.2,0.2)),
        strip.text = element_text(color = "white")) + 
...
png(file = "2023_10_20_Feminizer_CY_CA_AY_AA.png", width = 8, height = 8, units = "in", res = 1000)
pf
dev.off()


pdf(file = "2023_10_20_Feminizer_CY_CA_AY_AA.pdf", width = 8, height = 8)
pf
dev.off()