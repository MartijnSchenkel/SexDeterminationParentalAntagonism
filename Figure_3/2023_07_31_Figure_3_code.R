...
write.table(x = dm, file = "2023_07_31_data_3A.txt", row.names = F, col.names = T)
# ================= #
# ---- Figure 3A ----
# ================= #
...
d2 <- dm %>% dplyr::mutate(SD = str_sub(Haplotype, 1,1), PA = str_sub(Haplotype,2,2),
...
colors <- viridis_pal(option = "A")(9)
colors <- c(rgb(0,0.4,1),
            rgb(0.2,0.7,1),
            rgb(1,0.6,0.3),
            rgb(1, 0.8, 0.5))
...
pa <- d6 %>% 
  ...
  geom_line(size = 1) + 
  facet_wrap(~Sex) +
  scale_x_continuous(limits = c(0,max_t), expand = c(0,0),
...
# =================================== #
# ---- Simulate secondary invasion ----
# =================================== #

# Make sure everything is at equilibrium before introducing novel mutation.
# The first 10,000 generations will be removed as burn-in, so we'll move the
# mutation generation and the final generation back by 10,000
# Global parameters
max_t <- 70000
gen_skip <- 100
...
# Create arrays with sex determination info (effectively, we are assuming the ancestral pair
# is a ZW system (so the "Y" is actually a female-determining W) into which we introduce
# a novel male-determining A that is dominant over the XY SD locus).
IsFemale <- (nOne[1,,,,] > 0 & nOne[3,,,,] == 0)
IsMale <- 1 - IsFemale
...
for(l in 1:length(LG)) for(o in 1:length(Origin)) for(h in 1:length(Haplotype))
{
  ...
  df <- df %>% full_join(dt, by = names(dt))
}
...
write.table(x = df, file = "2023_07_31_data_3B.txt", row.names = F, col.names = T)
# ================= #
# ---- Figure 3B ----
# ================= #
...
p3 <- plot_grid(p2, NULL, nrow = 2, rel_heights = c(2,1), labels = c("", "C"))
# Check result
p3


# Save to PNG
png(file = "Figure_3.png", height = 8, width = 9, res = 1000, units = "in")
p3
dev.off()


# Save to PDF
pdf(file = "Figure_3.pdf", height = 8, width = 9)
p3
dev.off()
